<div class="flex h-full flex-col items-center justify-center">
  <svg width="82" height="82" viewBox="0 0 36 36"
    ><path
      fill="#66757F"
      d="M28.25 8.513a.263.263 0 0 0-.263-.263h-.475a.263.263 0 0 0-.263.263v11.475c0 .145.117.263.263.263h.475a.263.263 0 0 0 .263-.263V8.513z"
    /><g fill="#F19020"
      ><circle cx="27.75" cy="19.75" r="1.5" /><circle
        cx="27.75"
        cy="22.25"
        r="1"
      /></g
    ><path
      fill="#BD2032"
      d="M33.25 8.25h-4.129L9.946.29L9.944.289h-.001c-.016-.007-.032-.005-.047-.01C9.849.265 9.802.25 9.75.25h-.002a.511.511 0 0 0-.19.038a.522.522 0 0 0-.122.082c-.012.009-.026.014-.037.025a.483.483 0 0 0-.11.164V.56c-.004.009-.003.02-.006.029l-5.541 7.81c-.003.004-.003.01-.006.014a.985.985 0 0 0-.486.837v2a1 1 0 0 0 1 1h1.495L2.031 34H.25v2h18.958v-2h-1.74l-3.713-21.75H33.25a1 1 0 0 0 1-1v-2a1 1 0 0 0-1-1zm-21.769 4L9.75 13.639L8.02 12.25h3.461zM9.75 21.3l3.667 2.404l-3.667 2l-3.667-2L9.75 21.3zm-3.639.71l.474-2.784l1.866 1.223l-2.34 1.561zm4.938-1.561l1.87-1.225l.477 2.789l-2.347-1.564zm-1.299-.866l-2.828-1.885l2.828-2.322l2.828 2.322l-2.828 1.885zm-2.563-3.887l.362-2.127l1.131.928l-1.493 1.199zm3.633-1.198l1.132-.929l.364 2.13l-1.496-1.201zM5.073 8.25L9.25 2.362V6.25h-2a1 1 0 0 0-1 1v1H5.073zm.53 16.738l2.73 1.489l-3.29 1.794l.56-3.283zM15.443 34H4.067l.686-4.024L9.75 27.25l5.006 2.731l.687 4.019zm-1.54-9.015l.562 3.291l-3.298-1.799l2.736-1.492zM13.25 8.25v-1a1 1 0 0 0-1-1h-2V1.499L26.513 8.25H13.25zm2 3h-1.16v-2h1.16v2zm3 0h-2v-2h2v2zm3 0h-2v-2h2v2zm3 0h-2v-2h2v2zm3 0h-2v-2h2v2zm3 0h-2v-2h2v2zm3-.5a.5.5 0 0 1-.5.5h-1.5v-2h1.5a.5.5 0 0 1 .5.5v1z"
    /><path
      fill="#4B545D"
      d="M12.25 7.25h-2a1 1 0 0 0-1 1v2a1 1 0 0 0 1 1h3v-4h-1z"
    /><path fill="#CDD7DF" d="M11.25 7.25h2v4h-2z" /><path
      fill="#66757F"
      d="M34.844 24v-1H20.656v1h.844v2.469h-.844v1h14.188v-1H34V24z"
    /></svg
  >
  <span class="mt-20 flex items-center space-x-6">
    <span class="font-rubikheavy text-[40px] ">Error</span>
    <span class=" text-[40px]">|</span>
    <span class="text-[20px]">In development</span>
  </span>
  <a href="/" class=" mt-4 text-text-accent underline">Go home</a>
</div>
